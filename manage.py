...
from astgram.models import User, Image
from astgram.models import Comment
...
def init_database():
    ...
    for i in range(0, 100):
        db.session.add(User('User' + str(i + 1), '123' + str(i + 1)))
        for j in range(0, 3):
            db.session.add(Image(get_image_url(), j + 1))
            for k in range(0, 3):
    ...
    print (11, Image.query.get(1))
    print (12, Image.query.get(1).user)
